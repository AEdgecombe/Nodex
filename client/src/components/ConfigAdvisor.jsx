        ...
        <div className={STYLE.inputPanel}>
          <div className="space-y-6">
            <div>
              ...
              <select className={STYLE.inputField} value={formData.environment} onChange={e => setFormData({...formData, environment: e.target.value})}>
            ...
            <div>
              ...
              <input type="number" className={STYLE.inputField} value={formData.cpuCores} onChange={e => setFormData({...formData, cpuCores: parseInt(e.target.value)})} />
            </div>
            <div>
              <label className={STYLE.inputLabel}>Clock Speed (GHz)</label>
              <input type="number" step="0.1" className={STYLE.inputField} value={formData.clockSpeed} onChange={e => setFormData({...formData, clockSpeed: parseFloat(e.target.value)})} />
            </div>
            <div>
              <label className={STYLE.inputLabel}>Memory</label>
              <select className={STYLE.inputField} value={formData.ram} onChange={e => setFormData({...formData, ram: parseInt(e.target.value)})}>
            ...
            <div>
              ...
              <select className={STYLE.inputField} value={formData.storageType} onChange={e => setFormData({...formData, storageType: e.target.value})}>
                <option value="nvme">NVMe PCIe Gen4</option>
                <option value="ssd">SATA SSD</option>
                <option value="hdd">HDD</option>
              </select>
            </div>
            <button onClick={generateReport} disabled={isAnalysing} className={STYLE.submitBtn}>
          ...
          {!report ? (
            ...
            <div className="space-y-6 animate-in fade-in duration-1000 slide-in-from-right-8">
              <div className={STYLE.scorePanel}>
                ...
                <div className={`text-6xl font-extralight ${report.score >= 90 ? 'text-cyan-600 dark:text-cyan-400 drop-shadow-sm dark:drop-shadow-[0_0_15px_rgba(34,211,238,0.4)]' : report.score >= 60 ? 'text-violet-600 dark:text-violet-400' : 'text-red-500 dark:text-red-400'}`}>
                  {report.score}<span className="text-2xl text-slate-400 dark:text-white/20">/100</span>
                </div>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {[
                  { icon: Cpu, layer: 'Compute', res: report.components.cpu }, 
                  { icon: MemoryStick, layer: 'Memory', res: report.components.ram }, 
                  { icon: formData.environment === 'cloud' ? Cloud : HardDrive, layer: 'Storage', res: report.components.storage }
                ].map((item, i) => (
                  <div key={i} className={STYLE.metricCard}>
                    <div className="flex items-center gap-3 mb-4">
                      <div className={`${STYLE.metricBadgeBase} ${item.res.status === 'Optimal' ? 'bg-cyan-100 border-cyan-200 text-cyan-700 dark:bg-cyan-500/10 dark:border-cyan-500/20 dark:text-cyan-400' : 'bg-violet-100 border-violet-200 text-violet-700 dark:bg-violet-500/10 dark:border-violet-500/20 dark:text-violet-400'}`}>
              ...
              <div className={STYLE.scriptPanel}>
                <div className={STYLE.scriptHeader}>
                  ...
                  <button onClick={() => navigator.clipboard.writeText(generateCliScript())} className={STYLE.copyBtn}>Copy</button>
                </div>
                <div className="p-6 overflow-x-auto">
                  <pre className="text-xs font-light tracking-wide text-cyan-800 dark:text-cyan-300/60 leading-loose">{generateCliScript()}</pre>
                </div>
              </div>
